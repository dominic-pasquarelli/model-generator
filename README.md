...
What is real (host- and browser-level verified): the canonical semantic model (unknown/inferred/measured/confirmed value state — unknown is never zero), the pixel-to-millimetre calibration transform with plausibility rejection, the validation engine, a versioned schema with a migration harness, a **self-contained solid generator** that builds a **single connected, watertight, manifold** bracket (plate + bored standoffs + side tabs) from the model, **real ASCII STL and faceted B-rep STEP export** carrying a full auditable parameter sidecar, a **live orbitable 3D preview of that generated solid** whose dimensions and warnings come from the same live build, portable **`.mgproj` project files** (save/open, validated as an untrusted boundary), a **millimetre-fixed** geometry with a display-only **mm/inch** toggle, and version-monotonic **undo/redo**. The generated solid passes a **production, fail-closed** aggregate manifold audit *before* any preview or export is returned — one connected component, every edge shared by exactly two oppositely-oriented triangles, a single manifold fan at every vertex, and positive volume — and the sidecar carries the exact mm recipe plus a mesh hash that deterministically reconstructs the same solid. The STEP is a reference-complete, closed-shell ISO-10303-21 (AP214) part with one `MANIFOLD_SOLID_BREP`, validated against those internal structural properties rather than an independent EXPRESS/AP214 kernel.

What is **not** yet proven, and is deliberately honest about it: the STEP is a *faceted* B-rep (curved standoff walls are facets, not analytic surfaces) whose structure is checked against internal properties rather than an independent EXPRESS/AP214 kernel; the STL is a generated mesh whose **downstream slicer compatibility is not yet verified** against any real slicer; and **Autodesk Fusion import evidence and printed-part physical fit are not yet recorded** (see [ADR 0006](docs/decisions/0006-initial-export-format-contract.md)). Still deferred with a plan under [`docs/plans/`](docs/plans/): automatic board detection from an image, camera capture and skew-aware calibration, and the full accessibility/CI system. Nothing here should be read as proof that a board can be auto-detected from a photo, imported into CAD as a validated part, printed, or physically fitted.
