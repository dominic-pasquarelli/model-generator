...
This repository contains a **working Board Mount Designer** in `src/` (React + Vite + TypeScript, ADR 0003; owner-approved UI, ADR 0011), verified by host-level unit tests and a headless Playwright journey. What is Built and Verified (host/browser-level): the app shell and workflow, the canonical semantic model with an unknown/inferred/measured/confirmed value state, the pixel→mm calibration transform, validation, a versioned schema with a migration harness validated as an untrusted boundary, a **self-contained solid generator** (`src/core/geometry/mesh.ts`) that builds a **single connected, watertight, manifold** bracket (plate + bored standoffs + side tabs) with no boolean kernel, **real ASCII STL and faceted-B-rep STEP export** (one `MANIFOLD_SOLID_BREP`) carrying a full auditable parameter sidecar, a **live orbitable 3D preview of the actual generated solid** whose dimensions and warnings derive from the same live build, portable **`.mgproj` project files** (save/open), **millimetre-fixed** geometry with a display-only **mm/inch** toggle, and version-monotonic **undo/redo**. The generated solid passes a production, fail-closed aggregate manifold audit (`auditMesh`) — one component, every edge shared by exactly two oppositely-oriented triangles, a single manifold vertex fan, positive volume — before any preview or export is returned, and the sidecar's `geometryRecipe` deterministically reconstructs it (checked by `meshHash`). What is still **not** proven — and must not be implied as more than it is: the STEP is a *faceted* B-rep (curved walls are facets, not analytic surfaces) checked against internal structural properties rather than an independent EXPRESS/AP214 kernel; the STL is a generated mesh whose downstream slicer compatibility is unverified; and **Autodesk Fusion import evidence and printed-part fit are unrecorded** (ADR 0006); automatic board detection from an image, camera capture / skew-aware calibration, and the full accessibility/CI system remain deferred with plans under `docs/plans/`. Never claim CAD-import, print, or physical-fit success without the named evidence.
